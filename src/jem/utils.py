...
def get_super_source_flows(jem, as_list=True):
    ...
    if as_list is True:
        return list(
            jem.results_arcflows[
                (jem.results_arcflows.from_id == "super_source")
                & (jem.results_arcflows.flow > 0)
            ].to_id
        )
    else:
...
def get_nodal_edges(jem, node):
    return jem.edges.loc[(jem.edges.from_id == node) | (jem.edges.to_id == node)]
...
def get_flow_at_node(jem, node):
    ...
    return jem.results_arcflows.loc[
        (jem.results_arcflows.from_id == node) | (jem.results_arcflows.to_id == node)
    ]
...
def arc_indicies_as_dict(self, var_name):
    ...
    asDict = (
        self.edge_indices[self.indices + [var_name]]
        .set_index(keys=self.indices)[var_name]
        .to_dict()
    )
    return asDict
...
def add_super_source(nodes, edges):
    ...
    return pd.concat(
        [edges, new_edges], ignore_index=True
    )  # edges.append(new_edges, ignore_index=True)
...
def normalise_column(df_column):
    normalised_column = (df_column - df_column.min()) / (
        df_column.max() - df_column.min()
    )
