...
from jem.utils import get_nodal_edges, read_data, add_limits_to_edges, save_data
...
# read data
network = read_data(path_to_edges=path_to_edges, path_to_nodes=path_to_nodes)
...
# loop
network.nodes["capacity"] = network.nodes.progress_apply(
    lambda x: (
        nodal_capacity_from_edges(x["id"], network)
        if pd.isnull(x["capacity"])
        else x["capacity"]
    ),
    axis=1,
)
...
# save
save_data(network, path_to_edges=path_to_edges, path_to_nodes=path_to_nodes)
